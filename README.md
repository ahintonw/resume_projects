...
To create a true edge, I am now moving toward non-traditional data. By extracting natural language track notes (e.g., "opened up," "ridden out," "tired") from race docs, I can generate signals that aren't purely numerical. This shift inspired my work on a multi-layer political prediction system leveraging semantic NLP pipelines, RAG-based text parsing via LLM API integration, and temporal graph modeling across heterogeneous data sources (transcripts, flight logs, and financial signals) to forecast political outcomes ahead of prediction market consensus.
